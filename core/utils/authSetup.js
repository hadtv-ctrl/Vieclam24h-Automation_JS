...
async function createRuntimeUserData(parallelIndex) {
  ...
  if (!user) {
    throw new Error(`Worker ${parallelIndex} could not create an isolated registered user via API`);
  }
...
async function loginUserFromDataForPrecondition(page, providedUser = null, pageClasses = {}) {
  ...
  // Dismiss Onboarding modal overlay if it appears immediately following login (skip if spec explicitly tests onboarding)
  if (!pageClasses.skipCloseOnboarding) {
    try {
      const OnboardingClass = pageClasses.OnboardingPopupClass || OnboardingPopup;
      const onboarding = new OnboardingClass(page);
      await onboarding.closeIfVisible();
    } catch (_err) {
      // Onboarding modal may not appear
    }
  }
