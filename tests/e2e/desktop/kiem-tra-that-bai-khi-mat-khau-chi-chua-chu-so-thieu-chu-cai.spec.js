...
test.describe('Feature: Kiểm tra điều kiện mật khẩu khi đăng ký @register @desktop @e2e @REQ-001', () => {
  test('TC-029 - AC-001 Kiểm tra thất bại khi mật khẩu chỉ chứa chữ số (thiếu chữ cái)', async ({ page, pages }, testInfo) => {
    ...
    const passwordDigitsOnly = '12345678'; // 8 ký tự nhưng chỉ toàn chữ số, thiếu chữ cái
    ...
    await test.step('When [1] Nhập mật khẩu chỉ gồm chữ số nhưng không có chữ cái', async () => {
      await loginPopup.fillPassword(passwordDigitsOnly);
      await loginPopup.capture('password_digits_only_entered');
      // Trigger validation bằng cách blur hoặc bấm submit
      await loginPopup.clickSubmit().catch(() => null);
    });

    await test.step('Then [1] Hệ thống báo lỗi yêu cầu phải chứa ít nhất 1 chữ cái', async () => {
      await expect(loginPopup.passwordLetterError.or(loginPopup.passwordError)).toBeVisible();
      await loginPopup.capture('error_missing_letter_shown');
    });
  });
});
